...
一组纯前端、可离线运行的扑克研究工具，覆盖 MTT 卖股、Day 2 估值、现场赛程算账、资金管理、胜率计算和桌上复盘。所有计算都在浏览器本地完成，不上传任何数据。
...
| 工具 | 入口 | 功能 |
|---|---|---|
| 卖股报告生成器 | `tools/mtt-staking/#calc` | 生成 Markdown、HTML 和可打印 PDF 报告 |
| Day2 批量估值 | `tools/staking-calc/#batch-day2` | 粘贴名单、筹码和报价，输出排序表和 CSV |
| 赛事模板库 | `tools/tournament-templates/` | 保存常用 MTT / PKO / Mystery / FT Deal 参数 |
| 本地研究档案 | `tools/research-vault/` | 集中管理报告、批量记录和模板，可导出备份 |
