...
export const TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: '【选手视角策略 / Felix 方差模型】回答"我作为选手,在我的 bankroll 和场子下,**应该**卖多少股 / Markup 收多少最优 / 增长率多少"。考虑方差与 Kelly。需要 buyin / field / ROI / bankroll / markup。当用户问决策性问题(该不该 / 最优 / 推荐)时用这个。',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: '【投资人/经纪视角算账 / EV-only】回答"已经有这笔合约的 X/Y/Z 几个数,**算清**剩余的具体数字"。10 变量任意一部分填进去反推其它,不考虑方差/最优。当用户问"我赚多少 / 要打多少场 / 保本 markup 多少"这类**具体数字**问题时用这个。注意 s/R/Reff 传 0.5 不是 50。',
      parameters: {
        type: 'object',
        properties: {
...
- analyze_mtt_staking: 【选手策略】Felix 校准方差模型,回答"该卖多少 / Markup 多少最优"
- calc_staking_deal: 【投资人算账】10 变量定点求解,回答"赚多少 / 要打多少 / 保本 markup"等具体数字反推
- range_equity_mc:   范围对范围胜率蒙特卡洛

挑选 staking 工具的判断: 用户问"应该 / 最优 / 推荐 / 怎么定" → analyze_mtt_staking;
用户给已知数字问"赚多少 / 要打多少 / 反推" → calc_staking_deal。
